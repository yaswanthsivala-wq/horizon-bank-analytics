...
- **Full Pytest Regression Suite:**
  ...
  - Exactly 13 new unit and acceptance tests passing across 5 dedicated orchestration test modules:
    - [`tests/test_orchestration_runner.py`](../../../tests/test_orchestration_runner.py): 2 tests (AC-ORCH-01 end-to-end fixture execution, AC-ORCH-08 deterministic run_id and created_at_utc)
    ...
    - [`tests/test_orchestration_gating.py`](../../../tests/test_orchestration_gating.py): 3 tests (AC-ORCH-03 PUB-D01 DQ defect quarantine, AC-ORCH-03 reconciliation residual quarantine, AC-ORCH-04 K06 currency cohort candidate gating isolation)
  ...
  - **Reconciliation of Delivered Test Count (13 Delivered vs. 16–20 Estimated):**
    ...
    1. In [`tests/test_orchestration_runner.py`](../../../tests/test_orchestration_runner.py) (2 tests delivered vs. 4–5 estimated), `test_fixture_mode_end_to_end_orchestration_produces_all_marts` comprehensively evaluates AC-ORCH-01 end-to-end execution, curated entity production, customer risk assessments, all four analytical marts, and realistic metric assertions in a single cohesive execution, rather than splitting across multiple synthetic runs.
    2. In [`tests/test_orchestration_risk_flow.py`](../../../tests/test_orchestration_risk_flow.py) (2 tests delivered vs. 3–4 estimated), `test_risk_classification_t_u_rules_ac_orch_05` evaluates all four canonical $t/u$ risk profiles (Customers A, B, C, D) within a single unified fixture method, and `test_customer_risk_mart_segregates_incomplete_evidence` evaluates population segregation and non-additive exposure (AC-ORCH-05 and AC-ORCH-06).
    3. The remaining modules delivered exact planned test counts: [`tests/test_orchestration_mode_isolation.py`](../../../tests/test_orchestration_mode_isolation.py) (3 tests for AC-ORCH-02), [`tests/test_orchestration_gating.py`](../../../tests/test_orchestration_gating.py) (3 tests for AC-ORCH-03 and AC-ORCH-04), and [`tests/test_orchestration_replay.py`](../../../tests/test_orchestration_replay.py) (3 tests for AC-ORCH-07).
    All eight acceptance criteria (AC-ORCH-01 through AC-ORCH-08) are fully covered with zero gaps.
